...
export default function FinanciamientoPage() {
    ...
    useEffect(() => {
        const fetchCars = async () => {
            try {
                const query = `*[_type == "car"] | order(make asc) { make, model }`
                const cars = await client.fetch(query)
                const list = cars.map((c: any) => `${c.make} ${c.model}`)
                setAvailableCars(['Selecciona un vehículo', ...list, 'Otro / No está en la lista'])
            } catch (error) {
                setAvailableCars(['Error al cargar autos'])
            }
        }
        fetchCars()
    }, [])
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                    ...
                    <form onSubmit={handleSubmit} className="lg:col-span-7 order-2 lg:order-1">
                        <div className="bg-white rounded-[25px] border border-gray-100 p-7 space-y-9 shadow-none">
                            ...
                            <section className="space-y-6">
                                ...
                                <div className="space-y-5">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                        ...
                                        <InputField label="Renta Líquida Mensual" name="income" placeholder="Ej: 900.000" value={formData.income} onChange={handleChange} />
                                    </div>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            <section className="space-y-6">
                                ...
                                <div className="space-y-5">
                                    <FormSelect label="Auto de Interés" name="carInterest" value={formData.carInterest} options={availableCars} onChange={handleChange} />
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                        <InputField label="Monto de Pie" name="downPayment" placeholder="Ej: 5.000.000" value={formData.downPayment} onChange={handleChange} />
                            ...
                            <section className="space-y-6">
                                ...
                                <div className="flex flex-col space-y-2.5 text-left leading-none">
                                    <label className="text-[9px] font-black uppercase tracking-widest text-zinc-400 ml-1">¿Tienes alguna duda o comentario?</label>
                                    <textarea
                    ...
                    <aside className="lg:col-span-5 order-1 lg:order-2 space-y-6">
                        <div className="bg-white rounded-[25px] p-8 border border-gray-100 shadow-none">
                            <h4 className="text-[10px] font-black uppercase tracking-widest text-black mb-6 italic leading-none">Requisitos para el Crédito</h4>
                            <div className="space-y-6">
                                <FeatureItem title="Antigüedad" desc="Mínimo 1 año de continuidad laboral." />
                                <FeatureItem title="Renta Mínima" desc="Ingresos desde $500.000 líquidos mensuales." />
                            ...
                            <div className="mt-10 pt-6 border-t border-gray-100 text-center leading-tight">
                                <p className="text-[8px] font-bold text-zinc-400 uppercase tracking-widest mb-2">Cotiza en línea vía WhatsApp</p>
                                <p className="text-[10px] font-black text-black uppercase italic tracking-tight">Evaluación 100% Digital</p>
                            </div>
                        </div>
                        ...
                        <div className="bg-zinc-900 rounded-[25px] p-6 text-white shadow-none flex items-center gap-4">
                            ...
                            <div className="leading-tight">
                                <p className="text-[10px] font-black uppercase tracking-widest">Proceso Seguro</p>
                                <p className="text-[8px] text-zinc-500 font-bold uppercase tracking-tighter mt-0.5">Evaluación directa con VDL Motors</p>
                            </div>
                        </div>
                    </aside>
                </div>
...
function InputField({ label, name, placeholder, value, onChange, type = "text" }: any) {
    return (
        <div className="flex flex-col space-y-2.5 text-left leading-none">
            ...
            <input type={type} name={name} value={value} onChange={onChange} placeholder={placeholder} required className="w-full h-[42px] bg-[#F7F8FA] border-none rounded-xl px-5 text-[11px] font-bold outline-none focus:ring-1 focus:ring-black transition-all placeholder:text-zinc-300 placeholder:font-normal" />
        </div>
    )
}
    ...
    return (
        <div className="flex flex-col space-y-2.5 text-left leading-none">
            ...
            <div className="relative leading-none">
                <select name={name} value={value} onChange={onChange} className="w-full h-[42px] bg-[#F7F8FA] border-none rounded-xl px-5 py-0 text-[11px] font-black uppercase outline-none focus:ring-1 focus:ring-black appearance-none cursor-pointer leading-none">
