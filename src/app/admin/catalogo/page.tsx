...
import { toggleCarStatusAction, deleteCarAction } from '@/app/actions/carActions'
...
interface DashboardCar {
    ...
    slug?: string
    imageUrl?: string
    assignedSeller?: DashboardSeller | null
}
                ...
                <div>
                    ...
                    {car.assignedSeller && (
                        <div className="flex items-center gap-2 mt-4">
                            {car.assignedSeller.imageUrl ? (
                                <img
                                    src={car.assignedSeller.imageUrl}
                                    alt={`${car.assignedSeller.firstName || ''} ${car.assignedSeller.lastName || ''}`.trim()}
                                    className="w-5 h-5 rounded-full object-cover border border-gray-100 shrink-0"
                                />
                            ) : (
                                <div className="w-5 h-5 rounded-full bg-black text-white flex items-center justify-center text-[8px] font-black uppercase shrink-0">
                                    {(car.assignedSeller.firstName || car.assignedSeller.lastName || 'V').charAt(0)}
                                </div>
                            )}
                            <span className="text-[8px] font-bold uppercase tracking-widest text-zinc-500 truncate leading-none">
                                {`${car.assignedSeller.firstName || ''} ${car.assignedSeller.lastName ? car.assignedSeller.lastName.charAt(0) + '.' : ''}`.trim() || 'Vendedor'}
                            </span>
                        </div>
                    )}
                </div>
