...
export default function NuevoVehiculoPage() {
    ...
    const handleImageUpload = async (e: ChangeEvent<HTMLInputElement>, field: 'images' | 'exteriorImages' | 'interiorImages') => {
        ...
        try {
            ...
            console.error(error)
            alert("Error al subir. Asegúrate de que el Token de Sanity tenga permisos de 'Editor'.")
            ...
            <main className="max-w-7xl mx-auto px-6 py-8">
                <header className="flex justify-between items-end mb-9 gap-4">
                    <div className="text-left flex-1">
                        <p className="text-[8px] font-black text-zinc-400 uppercase tracking-[0.3em] mb-0.5 leading-none">Panel de administración</p>
                        <h1 className="text-2xl font-black uppercase tracking-tighter leading-none">Nuevo Vehículo</h1>
                    </div>
                    <div className="flex items-center gap-3">
                        <Link href="/admin/dashboard" className="text-[9px] font-black uppercase tracking-[0.2em] px-6 py-3 border border-zinc-200 rounded-xl hover:border-black transition-all">ir a inventario</Link>
                ...
                <form id="car-form" className="space-y-8">
                    ...
                    <div className="bg-white rounded-[30px] border border-gray-100 p-7 space-y-5 shadow-none">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-7">
                            ...
                            <FormSelect label="Etiqueta (Badge)" value={formData.category} options={['Seminuevo', 'Recién Llegado', 'Oportunidad', 'Vendido']} onChange={(v) => handleChange('category', v)} />
                            <FormGroup label="Precio Lista (Contado)" type="number" value={formData.listPrice} onChange={(v) => handleChange('listPrice', parseInt(v) || 0)} />
                            <FormGroup label="Precio Financiado (Bono)" type="number" value={formData.financedPrice} onChange={(v) => handleChange('financedPrice', parseInt(v) || 0)} />
                    ...
                    <div className="bg-white rounded-[30px] border border-gray-100 p-7 space-y-5 shadow-none">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-7">
                            <div className="md:col-span-2"><FormGroup label="Motor (Cilindrada/Potencia)" value={formData.engine} onChange={(v) => handleChange('engine', v)} /></div>
                    ...
                    {/* BLOQUES 3 AL 6: ESPECIFICACIONES */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="bg-white rounded-[30px] border border-gray-100 p-7 space-y-5 shadow-none">
                            <h3 className="text-[10px] font-black uppercase tracking-widest text-zinc-700 border-b border-gray-50 pb-4 leading-none">Especificaciones: General</h3>
                            <FormGroup label="Cilindrada (Ej: 1.6L)" value={formData.specsGeneral.cilindrada} onChange={(v) => handleNestedChange('specsGeneral', 'cilindrada', v)} />
                            <FormGroup label="Cilindros (Ej: 4)" value={formData.specsGeneral.cilindros} onChange={(v) => handleNestedChange('specsGeneral', 'cilindros', v)} />
                            <FormGroup label="Potencia (Ej: 110 HP)" value={formData.specsGeneral.potencia} onChange={(v) => handleNestedChange('specsGeneral', 'potencia', v)} />
                        </div>
                        <div className="bg-white rounded-[30px] border border-gray-100 p-7 space-y-5 shadow-none">
                            <h3 className="text-[10px] font-black uppercase tracking-widest text-zinc-700 border-b border-gray-50 pb-4 leading-none">Especificaciones: Historial</h3>
                            <FormGroup label="Dueños (Ej: Único dueño)" value={formData.specsHistory.duenos} onChange={(v) => handleNestedChange('specsHistory', 'duenos', v)} />
                            <FormGroup label="Mantenciones (Ej: Al día)" value={formData.specsHistory.mantenciones} onChange={(v) => handleNestedChange('specsHistory', 'mantenciones', v)} />
                    ...
                    {/* BLOQUE 7: DETALLES ADICIONALES (EQUIPAMIENTO COMPLETO) */}
                    <div className="bg-white rounded-[30px] border border-gray-100 p-7 space-y-5 shadow-none">
                    ...
                    {/* BLOQUE 8: MULTIMEDIA Y DESCRIPCION */}
...
// --- AUXILIARES TIPADOS ---
interface FGProps { label: string; value: any; onChange: (v: string) => void; type?: string; placeholder?: string; }
...
interface FSProps { label: string; value: any; options: string[]; onChange: (v: string) => void; }
...
function ImageUploadPlaceholder({ label, images, field, onClick, onRemove }: any) {
    return (
        <div className="flex flex-col space-y-3 text-left leading-none">
            ...
            {/* BOTÓN DE CARGA SIEMPRE VISIBLE ARRIBA */}
            <div onClick={onClick} className="aspect-video bg-[#F7F8FA] border-2 border-dashed border-zinc-100 rounded-[25px] flex flex-col items-center justify-center group hover:border-black transition-all cursor-pointer">
                ...
                <p className="text-[8px] font-black uppercase tracking-widest text-zinc-400">Añadir Imagen</p>
            </div>

            {/* MINIATURAS APARECEN ABAJO */}
            {images.length > 0 && (
                <div className="grid grid-cols-2 gap-3 mt-2">
                    {images.map((img: any, i: number) => (
                        <div key={i} className="relative aspect-video group leading-none">
